...
apply_bundle() {
    ...
    if [ "$github_count" -gt 0 ] && [ "$run_github" -eq 1 ]; then
        ...
        # Toolchains the github BUILDS depend on must exist BEFORE this phase.
        # The pip (Phase 3) and cargo (Phase 4) phases run AFTER github, so:
        #   - Rust tools fail with "cargo: command not found"
        #   - Python tools fail with "/opt/pentest-venv/bin/pip: No such file"
        # Set both up here up-front.
        if ! command -v cargo >/dev/null 2>&1 && [ ! -x /root/.cargo/bin/cargo ]; then
            print_status "  Installing rust/cargo (Rust github tools need it to compile)..."
            apt-get install -y -q cargo rustc >/dev/null 2>&1 || true
        fi
        [ -x /root/.cargo/bin/cargo ] && export PATH="/root/.cargo/bin:$PATH"
        if [ ! -x /opt/pentest-venv/bin/pip ]; then
            print_status "  Creating /opt/pentest-venv (Python github tools install into it)..."
            python3 -m venv /opt/pentest-venv >/dev/null 2>&1 && \
                /opt/pentest-venv/bin/pip install -q --upgrade pip setuptools wheel >/dev/null 2>&1 || true
        fi
        ...
        # 2b) Bundle loop installs every bundle tool at its declared target path
        # (which is what `portalgun verify` checks). The curated installer (2a)
        # already produced proper binaries for the tools it owns; the skip-if-
        # present check below means we only clone what isn't already on disk, so
        # tools the curated step placed at the SAME path aren't re-cloned, while
        # tools it installed under a different name (e.g. chisel_linux) still get
        # populated at their bundle path so verify counts them.
